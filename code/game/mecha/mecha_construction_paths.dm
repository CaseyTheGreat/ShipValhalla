...
/datum/construction/mecha/segway_assembly
	steps = list(list("key"=/obj/item/mecha_parts/part/segway_handlebars),//1
					 list("key"=/obj/item/mecha_parts/part/segway_wheel_left),//3
					 list("key"=/obj/item/mecha_parts/part/segway_wheel_right)
	...
	spawn_result()
		var/obj/item/mecha_parts/chassis/const_holder = holder
		const_holder.construct = new /datum/construction/reversible/mecha/segway(const_holder)
		const_holder.density = 1
		spawn()
			del src
		feedback_inc("mecha_segway_created",1)
		return



/datum/construction/reversible/mecha/segway
	result = "/obj/mecha/working/segway"
	steps = list(
					//1
					list("key"=/obj/item/weapon/weldingtool,
							"backkey"=/obj/item/weapon/weldingtool,
							"desc"="The handlebars are welded to the frame."),
					 //2
					 list("key"=/obj/item/weapon/wrench,
					 		"backkey"=/obj/item/weapon/wrench,
					 		"desc"="The left wheel is fixed to the axle."),
					 //3
					 list("key"=/obj/item/weapon/wrench,
					 		"backkey"=/obj/item/weapon/wrench,
					 		"desc"="The right wheel is fixed to the axle."),
					 //4
					 list("key"=/obj/item/weapon/wirecutters,
					 		"backkey"=/obj/item/weapon/cable_coil,
					 		"desc"="The wiring is clipped and adjusted."),
					 //5
					 list("key"=/obj/item/weapon/cable_coil,
					 		"backkey"=/obj/item/weapon/screwdriver,
					 		"desc"="The wiring is inserted."),
	...
	custom_action(index, diff, atom/used_atom, mob/user)
		...
		switch(index)
			if(1)
				user.visible_message("[user] connects [holder] handlebars.", "You connect [holder] handlebars.")
			if(3)
				if(diff==FORWARD)
					user.visible_message("[user] connects [holder] right wheel.", "You connect [holder] right wheel.")
				else
					user.visible_message("[user] disconnects [holder] right wheel.", "You disconnect [holder] right wheel.")
			if(2)
				if(diff==FORWARD)
					user.visible_message("[user] connects [holder] left wheel.", "You connect [holder] left wheel.")
				else
					user.visible_message("[user] disconnect [holder] left wheel.", "You disconnect [holder] left wheel.")
			if(4)
				if(diff==FORWARD)
					user.visible_message("[user] adjusts and clips [holder] wiring.", "You adjust and clip [holder] wiring.")
				else
					user.visible_message("[user] cuts [holder] wiring.", "You cut [holder] wiring.")
			if(5)
				if(diff==FORWARD)
					user.visible_message("[user] adds [holder] wiring.", "You add [holder] wiring.")
				else
					user.visible_message("[user] removes [holder] wiring.", "You remove [holder] wiring.")
		return 1
